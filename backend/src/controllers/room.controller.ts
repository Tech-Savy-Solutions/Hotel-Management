...
export const getRoomById = (req: any, res: any) => {
    Room.findById(req.params.id)
        .then((room) => {
            if (!room) {
                return res.status(404).json({ noRoomFound: 'No Room found' });
            }
...
export const addRoom = (req: any, res: any) => {
    Room.create(req.body)
        ...
        .catch((err) =>
            res.status(400).json({ error: 'Unable to add this Room', err })
        );
};
...
export const deleteRoom = (req: any, res: any) => {
    Room.findByIdAndDelete(req.params.id)
        ...
        .catch((err) => res.status(404).json({ error: 'No such a Room', err }));
};
